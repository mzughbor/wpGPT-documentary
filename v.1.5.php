...
function chatgpt_ava_private_rewrite()
{
    ...
    */
    ...
    // OLD //set title and description for seo
    function change_yoast_seo_title_and_meta_description($post_id, $generated_keyphrase)
    {
        // Get the current post or page title and meta description.
        $post_title = get_the_title($post_id);
        error_log('-- 1 --' . $post_title ."\n", 3, CUSTOM_LOG_PATH);
        $post_meta_description = get_post_meta($post_id, '_yoast_wpseo_meta_description', true);
        error_log('-- 2 --' . $post_meta_description ."\n", 3, CUSTOM_LOG_PATH);

        // Strip tags for the generated keyphrase.
        $generated_keyphrase = strip_tags($generated_keyphrase);
        error_log('-- 2.01 --' . $generated_keyphrase ."\n", 3, CUSTOM_LOG_PATH);
        //$generated_keyphrase = addslashes($generated_keyphrase);
        $generated_keyphrase = str_replace('.', '\.', $generated_keyphrase);
        error_log('-- 2.02 --' . $generated_keyphrase ."\n", 3, CUSTOM_LOG_PATH);

        // Set the new SEO title and meta description.
        $title = $generated_keyphrase . ' | wedti ';
        error_log('-- 2.03 --' . $title ."\n", 3, CUSTOM_LOG_PATH);

        $title_length = strlen($title);
        error_log('-- 3 --' . $title_length ."\n", 3, CUSTOM_LOG_PATH);

        // If the title is longer than 72 characters, trim it.
        if ($title_length > 72) {
            $title = substr($title, 0, 72);
            error_log('-- > 72 --' . $title ."\n", 3, CUSTOM_LOG_PATH);
        } else {
        // If the title is less than 72 characters, fill the remaining area with some of text from post title itself and make last three characters like this 3 dots '...'
            $remaining_characters = 72 - $title_length;
            error_log('-- < 72 --' . $remaining_characters ."\n", 3, CUSTOM_LOG_PATH);            
            $post_title_words = explode(' ', $post_title);
            error_log('-- < 72 --' . print_r($post_title_words, TRUE)."\n", 3, CUSTOM_LOG_PATH);            
            $last_word = array_pop($post_title_words);
            error_log('-- < 72 --' . $last_word ."\n", 3, CUSTOM_LOG_PATH);                        
            $title .= ' ' . $last_word . str_repeat(' ', $remaining_characters - 3) . '...';
            error_log('-- < 72 --' . $title ."\n", 3, CUSTOM_LOG_PATH);                        

        }

        //$description = generate_meta_description($generated_keyphrase);
        $description = 'ops..';
        
        // Update the post meta.
        $post_meta = array(
            '_yoast_wpseo_title' => $title,
            '_yoast_wpseo_meta_description' => $description,
        );
        update_post_meta($post_id, $post_meta);

                //$set_keyphrase_result = update_post_meta($post_id, '_yoast_wpseo_focuskw', $generated_keyphrase);

        error_log('-- 4 -- post updated' . "\n", 3, CUSTOM_LOG_PATH);                        

        // Return the new SEO title and meta description.
        return array($title, $description);

    }
    //add_filter('wpseo_title', 'change_yoast_seo_title_and_meta_description', 10, 2);
    //add_filter('wpseo_meta_description', 'change_yoast_seo_title_and_meta_description', 10, 2);
    ...
    function update_yoast_seo_title($post_id, $new_title)
    {
        ...
        if (defined('WPSEO_VERSION')) {
            ...
            error_log("undefined WPSEO_VERSION..."."\n", 3, CUSTOM_LOG_PATH);
        }
    }
        ...
        // Rest of the code to generate and update content based on the API response...
            ...
            */
            ...
            elseif (!count_and_manage_posts($post->ID, $filterd_content) ){
                ...
                // future update to stop this if post is acaully deleted due to prevouis erroe 
                
                error_log('generated_keyphrase ::' . print_r($generated_keyphrase, true)."\n" , 3, CUSTOM_LOG_PATH);
                
                // old
                
                //change_yoast_seo_title_and_meta_description($post->ID, $generated_keyphrase);
                
                // new
                
                $SEO_title = "write SEO title for prevouis article with using of exact keyphrase \"{$generated_keyphrase}\" with limit of 70 character in total using the Arabic language. make sure to add name of site \" Wedti.com \" in the tilte.";
