...
function chatgpt_ava_settings_page()
{
    ...
    <div class="wrap">
        ...
        <h2>Draft Posts List</h2>
        <p>Below is the list of draft posts with less than <?php echo $min_word_count; ?> words:</p>
...
function chatgpt_ava_private_rewrite()
{
    ...
    */
    ...
    function count_and_manage_posts($post_id) {
        $min_word_count = 130;
        ...
        if ($post && $post->post_type === 'post') {
            ...
            $word_count = str_word_count( preg_replace("/[\x{0600}-\x{06FF}a-zA-Z]/u", "a", strip_tags($post->post_content)) );
            if ($word_count < $min_word_count) {
                error_log('-- inside 2`ed if, here is $word count :'. $word_count ."\n", 3, CUSTOM_LOG_PATH);
                ...
                error_log('-- inside else if 2:'."\n", 3, CUSTOM_LOG_PATH);
                return true;
            }
        } else {
            error_log('-- inside else if 1:'."\n", 3, CUSTOM_LOG_PATH);
            return false;
        }
    }
    ...
    foreach ($private_posts as $post) { 
        ...
        // Rest of the code to generate and update content based on the API response...
        } elseif (count_and_manage_posts($post->ID)) {
            ...
            error_log('The count_words function: ' . print_r($word_count, true)."\n", 3, CUSTOM_LOG_PATH);
